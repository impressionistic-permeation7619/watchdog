...
import { formatLinksMarkdownSection, mergeUnique } from "@watchdog/tools";

import { ingestRemotePage } from "./ingest-page";
...
export const networkUrlEnrich = defineCapability({
  ...
  async run(ctx) {
    ...
    const live = await ingestRemotePage({
      fetchUrl: url,
      linkBaseUrl: url,
      signal: ctx.signal,
      label: "live",
    ...
    const ts = await closestWaybackTimestamp(url, ctx.signal);
    if (ts !== null && ts !== "") {
      const archiveUrl = waybackArchiveUrl(ts, url);
      wayback = await ingestRemotePage({
        fetchUrl: archiveUrl,
        linkBaseUrl: url,
        signal: ctx.signal,
        label: "wayback",
    ...
    if (!combined) {
      throw new Error(
        `URL enrich found no usable content (live: ${live.step.error ?? live.step.status}; wayback: ${wayback.step.error ?? "n/a"})`
      );
    }
