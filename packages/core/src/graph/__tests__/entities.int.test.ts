import { beforeEach, describe, expect, it } from "vitest";

import { createEntity, listQuestionsForEntity } from "@watchdog/core";
import { db } from "@watchdog/db";
import { resetTestDb, seedCase } from "@watchdog/test-kit/db";

describe("createEntity", () => {
  beforeEach(async () => {
    await resetTestDb();
  });

  it("seeds default questions for a person in the same transaction", async () => {
    const cased = await seedCase(db);
    const person = await createEntity({
      caseId: cased.id,
      kind: "person",
      name: "Ada Lovelace",
      slug: "ada-lovelace",
    });
    const questions = await listQuestionsForEntity(cased.id, person.id);
    expect(questions.length).toBeGreaterThan(0);
    expect(questions.every((q) => q.status === "open")).toBe(true);
  });

  it("does not seed questions for an org", async () => {
    const cased = await seedCase(db);
    const org = await createEntity({
      caseId: cased.id,
      kind: "org",
      name: "Analytic Engine",
      slug: "analytic-engine",
    });
    const questions = await listQuestionsForEntity(cased.id, org.id);
    expect(questions).toHaveLength(0);
  });
});
