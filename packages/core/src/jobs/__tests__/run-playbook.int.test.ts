...
import { cancelPlaybookRun, dumpUrl, runPlaybook } from "@watchdog/core";
...
describe("cancelPlaybookRun", () => {
  ...
  it("cancels the run and its cancellable members", async () => {
    const cased = await seedCase(db);
    const started = await runPlaybook({
      caseId: cased.id,
      playbookId: "host-footprint",
      actorId: TEST_ACTOR_ID,
      seed: { host: "mailhost.test" },
    ...
    const cancelled = await cancelPlaybookRun(cased.id, started.playbookRunId);
