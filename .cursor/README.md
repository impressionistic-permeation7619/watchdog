...
| `afterFileEdit` | [`validate-on-edit.mjs`](hooks/validate-on-edit.mjs) via [`run-node.sh`](hooks/run-node.sh) | No | Re-runs `check-agents.mjs --strict` on `AGENTS.md` edits, or `validate-agents.mjs` on edits under [`.agents/skills/`](../.agents/skills/) or this file. Catches a broken skill or doc the moment it's introduced. Failures print to stderr only — the `stop` hook is what surfaces findings to the agent. |
| `stop` | [`stop-gate.mjs`](hooks/stop-gate.mjs) via [`run-node.sh`](hooks/run-node.sh) | No | Scoped to files changed this turn: lints them (`oxlint`, filtered from a repo-wide run — type-aware mode needs the whole project graph), runs `ds:ban` if web UI paths are dirty, runs the agent-skills / `AGENTS.md` validators if those paths are dirty. `loop_limit: 2` — surfaces a finding at most twice per turn, then lets the agent stop rather than looping forever on something it can't fix. |
