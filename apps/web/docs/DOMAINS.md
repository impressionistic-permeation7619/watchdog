...
**Dossier section chrome:** shared create/edit state lives in `dossier/hooks/use-dossier-section-editor.ts` (`adding` / `editId` / `error` + empty gate). Shell data (counts, evidence options, rename/edit mutations, live invalidation) lives in `dossier/hooks/use-dossier-shell.ts`. Export actions: `dossier/components/dossier-export-menu.tsx`. Ghost / panel Add CTAs use `dossier/components/dossier-section-add-button.tsx`. Evidence tab dump (File/Paste/URL, Entity locked) lives in `entity-evidence-section.tsx` via Intake `useDumpEvidence` + `DumpDialogs` — no `dossier.functions.ts`. Confirmed↔evidence Accept gate + copy: `dossier/lib/confirmed-evidence.ts` (also used by Inbox + connection dialog). Claim create/edit share `dossier/lib/claim-form.ts` (`formOptions`) via one `ClaimComposer`. Section Graph writes (claims / questions / identifiers / events / connections) use `useMutation` — do not leave bare `await …Fn()` siblings beside mutation-backed create/update.
...
- Edge RPC stays `entities/edges/` (entity-scoped + case-wide `listForCase`). Shared xyflow chrome: `shared/ui/graph/`. Case graph page: `cases/components/case-graph/*` + `graph-page.tsx` (`/graph`).

## Rules

1. One noun = one product concern. Don’t put Jobs UI under Inbox.
...
4. Handlers call **`orpcForActor(actorFromSession(...)).…`** for domain I/O (same pattern as `jobs` / `inbox` / graph children).
...
7. Domains with RPC inputs keep Zod in `types.ts` (jobs/inbox included).
...
| --- | --- | --- | --- | --- |
| `jobs` | Cap Jobs: list, get (detail/logs), start, cancel, artifact text | `/jobs` | `jobs.tsx` → `JobDetail` (`lib/job-detail-view.ts` + `TabsContent`/`ActiveTabBody`); Cap/Playbook = `ToggleGroup` mode selector; workspace `hooks/use-jobs-workspace.ts` (optimistic list/detail seed on start so `?jobId=` does not Navigate-flicker); run forms `job-cap-run-form.tsx` / `job-playbook-run-form.tsx` (+ `lib/cap-run-view.ts` / `lib/playbook-seed-view.ts` seeds host/url/evidence/ip/email/hash/handle / CapMatch `lib/cap-match.ts` + `cap-capability-select.tsx`); queue grouping `lib/status.ts` (`playbookRunStatus`, waiting chrome for the next recipe step — not Job `blocked`) | `jobs.functions.ts` + `queries.ts` (`jobsListQuery` / `jobDetailQuery`) |
| `inbox` | Proposals: accept / reject triage | `/inbox` | `inbox.tsx` → `InboxDetail` → `inbox-decide-header.tsx` + `inbox-patch-body.tsx` (shared `lib/decide-header-view.ts` `decideMode`); identifier collision Alert + per-op chip (warn, don’t block); invalid Identifier ops via schemas `listInvalidIdentifierOps` (chip + **disable Accept**); workspace `hooks/use-inbox-workspace.ts` (filters, selection, accept/reject, SSE); forms `hooks/use-inbox-detail-forms.ts`; Accept gate `lib/accept-validation.ts` + `dossier/lib/confirmed-evidence.ts`; Accept composer DTO `AcceptFormValues` in `types.ts` (not under `components/`) | `inbox.functions.ts` + `types.ts` + `queries.ts` |
| `intake` | Evidence dump, Enrich/Process verbs, queue (Content · Output · Jobs) | `/intake` | `intake.tsx` → `EvidenceDetail` (`TabsContent` + `ActiveTabBody`; dump mutations `hooks/use-dump-evidence.ts`; actions from `hooks/use-intake-actions.ts` pending discriminant); header `evidence-detail-header.tsx` (Detail EntityCombobox attach/replace); blob `hooks/use-evidence-blob.ts` | `intake.functions.ts` + `types.ts` + `queries.ts` |
...
| `dashboard` | Dashboard (`/`) — stats + Inbox + Due + resizable Activity (`ScrollArea`) | `/` | `dashboard-home.tsx` (`Page density="split"` + vertical `ResizablePanelGroup`), `metrics-section.tsx`, `dashboard-panels.tsx`, `recent-activity.tsx`, `lib/selectors.ts` | Composes other domains’ queries (+ `recentActivityQuery`) |
...
| --- | --- | --- |
| **Split-view** (Jobs, Inbox, Intake) | `<Page density="split">` | `PageHeader` + `SplitView` (Queue + Detail) |
| **Table** (`/entities`, `/identifiers`) | — (thin route) | `<Page>` + `PageHeader` + `DataTable` (`entity-table.tsx` / `identifiers-page.tsx`; table state in `hooks/use-entity-table.ts` / `hooks/use-identifiers-table.ts`). Column `size` required — [`UI.md`](UI.md) Table columns. Bulk-add preview is a raw `Table` + `<colgroup>` in the dialog. |
| **Board** (`/tasks`) | — (thin route) | `<Page>` + `PageHeader` + `TaskBoard` via `tasks-page.tsx` + `useTaskWorkspace` |
| **Stack** (Dossier, Case Overview, Cases list, Graph) | — (thin route; dossier/case not-found only) | Full `<Page>` shell (`dossier.tsx`, `case-overview.tsx`, `case-list.tsx`, `graph-page.tsx`) |
| **Mixed / split stack** (Dashboard) | — (thin route) | `dashboard-home.tsx`: `Page density="split"`; vertical resizable overview ↔ Activity |
| **Settings** | `<Page>` + `PageHeader` | `SettingsShell` + tab panels only |

Split-view domains must not wrap a second `<Page>`. Table/board/stack domains own the shell in the domain entry (`actions=`, `count=` + `countOn=` on table/board last crumbs, and `below=` line tabs live with the tab state they drive). Identity is the PageHeader trail — see [`UI.md`](UI.md) trail notes. Do not pass per-page identity titles or explainer `description=` (404 missing-slug copy only).
...
| Jobs workspace | `useJobsWorkspace(caseId, { jobId, onJobIdChange, …, jobsListFetching? })` owns selection, detail fetch, cap/playbook start/cancel, SSE `job_update`. On start: seed list+detail cache before `onJobIdChange` so URL selection resolves without a Navigate remount. Auto-fallback uses `resolveQueueSelection` (`holdMissingUrlId` while list refetching or job exists but filtered out of queue) + render-time `<Navigate replace>` (same as Inbox/Intake — not a sync `useEffect`). Run UI: `JobCapRunForm` (paste-to-run CapMatch + empty-default Cap select + filters) / `JobPlaybookRunForm`; vault `credentialsListQuery` folded into `canRun`; run-mode `ToggleGroup` lives in `jobs.tsx`. Do not fork a second jobs mutation machine. |
| Inbox workspace | `useInboxWorkspace(caseId, { proposalId, initialStatus })` owns filters, selection, accept/reject mutations, SSE `proposal_created`. First paint = pending-only (`PENDING_INBOX_FILTERS`). URL auto-fallback via `<Navigate replace>` + `resolveQueueSelection`. Do not fork a second inbox mutation machine. |
...
| Dashboard | Composes inbox / tasks / jobs / entities / activity queries; selection helpers in `dashboard/lib/selectors.ts`. Activity = vertical resizable panel + `ScrollArea` (`recent-activity.tsx`). Do not put dump/paste on Dashboard — Intake owns dump. |
