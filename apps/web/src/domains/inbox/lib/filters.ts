...
export interface InboxQueueFilters {
  q: string;
...
export const EMPTY_INBOX_FILTERS: InboxQueueFilters = {
  q: "",
  statuses: [],
};

/** Pending-only — first paint and after accept/reject / new proposal. */
export const PENDING_INBOX_FILTERS: InboxQueueFilters = {
  q: "",
  statuses: ["pending"],
};

/** Pending-only facet — empty queue here is “cleared”, not “no results”. */
export function isInboxPendingOnlyFilters(filters: InboxQueueFilters): boolean {
  return (
    !filters.q.trim() &&
...
export function proposalEntityName(row: ProposalRecord): string | null {
  const { patch, entityNames = {} } = row;
  const entityId = patch
    .map((op) => patchOpEntityId(op))
    .find((id): id is string => id !== undefined);
  if (!entityId) return null;
  const name = entityNames[entityId];
