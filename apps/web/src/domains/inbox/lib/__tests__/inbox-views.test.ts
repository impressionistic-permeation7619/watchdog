...
describe("inbox filters", () => {
  it("defaults to pending-only", () => {
    expect(isInboxPendingOnlyFilters(PENDING_INBOX_FILTERS)).toBe(true);
    expect(isInboxPendingOnlyFilters(EMPTY_INBOX_FILTERS)).toBe(false);
    const pending = proposal({ status: "pending" });
    const accepted = proposal({ id: testId(51), status: "accepted" });
    expect(
      filterInboxQueue([pending, accepted], PENDING_INBOX_FILTERS).map(
        (row) => row.id
      )
    ).toEqual([pending.id]);
  });
