...
type Stage = "paste" | "map";

function parsePasteTarget(value: string): IdentifierPasteTarget | null {
...
export function BulkAddIdentifiersDialog({
  ...
  const importMutation = useMutation({
    ...
    onSuccess: async (result, vars) => {
      ...
      setPaste(
        rebuildIdentifierPaste({ table: vars.table, keepSourceIndices: keep })
      );
      setRowOverrides(new Map());
      setServerErrors(
        new Map(result.failed.map((row) => [row.key, row.message]))
      );
    },
  });
